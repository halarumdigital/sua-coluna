...
async function fixAgentBindingUltimate() {
  ...
  try {
    ...
    // 1. Verificar estrutura da tabela franchises
    console.log('1️⃣ Verificando estrutura da tabela franchises...');
    const [franchiseColumns] = await connection.execute('DESCRIBE franchises');
    ...
    // 4. Verificar franchises disponíveis
    console.log('4️⃣ Verificando franchises disponíveis...');
    ...
    console.log(`   ✅ Franchise encontrada: ${franchise.name} (ID: ${franchise.id})`);
    ...
    if (existingBindings.length > 0) {
      ...
      // Usar o user_id do agente ou da franchise
      const finalUserId = agent.user_id || franchise.user_id;
